    ...
    try {
        ...
        if (type === 'experience-to-post') {
            ...
            const prompt = `
                ...
                우리의 브랜드 보이스는 다음과 같다:
                "우리는 지식을 가르치지 않습니다. 알게 된 순서를 기록합니다. 상품을 설명하기보다 어떤 선택이 어떤 하루를 만들었는지를 기록합니다. 확신 대신 회고를, 결론 대신 질문을 남깁니다."
                ...
                - 정보형 (에세이형 정보 템플릿 적용):
                    1. 시작 문장: 질문이 아닌 계기 ("이걸 처음부터 알았던 건 아니었습니다. 오히려 몰라서 헤맸던 쪽에 가까웠어요.")
                    2. 이전의 생각: 흔한 오해 또는 나의 착각 (독자를 가르치려 하지 말고, 나 역시 했던 실수를 공유하며 신뢰 형성)
                    3. 핵심 깨달음: 문제의 본질 ("핵심은 정보의 양이 아니라, 순서였습니다.")
                    4. 구조화된 정보: 최소한의 정리 (리스트/번호 사용 가능하나 교과서 말투 금지. "알아야 할 목록"이 아닌 "정리해둔 메모" 느낌으로)
                    5. 적용 맥락: '누구에게 유용한가' 대신 '언제 떠오르는가' 제시
                    6. 한계와 예외: 정보의 절대성 제거 (항상 맞지 않을 수도 있음을 솔직히 언급)
                    7. 마무리 문장: 결론이 아닌 여운 ("지금은 이 정도로 충분합니다. 나머지는 필요할 때 찾아보면 되니까요.")
                - 커머스형 (에세이형 커머스 템플릿 적용):
                    1. 첫 문장: 쓰게 된 이유 (후킹이 아닌 고백. "어느 날, 이게 필요하다는 생각이 들었습니다.")
                    2. 망설임: 바로 사지 않았다는 기록 (충동구매가 아님을 보여줌)
                    3. 선택의 이유: 기능을 경험으로 번역 (예: "오후까지는 신경 쓸 일이 없었습니다")
                    4. 사용 후 변화: 하루의 변화 (과장 금지. "하루가 조금 덜 번거로워졌어요")
                    5. 솔직한 배제: 안 맞을 수도 있는 사람 (단점 언급)
                    6. 마지막 문장: 추천이 아닌 기록의 끝 ("다시 고르라고 해도 같은 선택을 할 것 같아요")
                    7. CTA: 판매 버튼이 아닌 여백 문장 ("이 선택이 궁금하다면, 아래에 남겨두었습니다.")

                [작성 로직]
                STEP 1. 사례 구조화: 상황-행동-불편-전환점-여운 구조로 정리.
                STEP 2. 사례 진짜성 검증: 인위적인 느낌 제거.
                STEP 3. 유형별 리프레이밍: ${contentType}에 맞는 관점 설정. (커머스/정보형은 각 전용 템플릿 준수)
                STEP 4. 톤 앵커 고정: 1인칭 유지. 확신하는 어미("반드시/꼭/필수") 제거. 느낌표 과다 금지.
                STEP 5. 초안 작성: "저는 / 당시엔 / 나중에 보니" 등 과정과 실수, 수정을 기록하는 어조.
                STEP 6. 「했어요, 그랬나요?」 필터링 적용: 
                    - "했어요", "그랬나요?", "잘 모르겠어요", "그런 것 같기도 해요" 등의 어미를 자연스럽게 섞을 것.
                    - 정답을 말하기보다 깨달음에 도달하는 과정을 보여줄 것.
