        ...
        if (type === 'experience-to-post') {
            ...
            const prompt = `
                ...
                - 이슈형 (에세이형 이슈 템플릿 적용):
                    ...
                    3. 흔들린 계기: 생각이 바뀐 지점 (어떤 문장이나 데이터를 보고 생각이 달라졌는지 기록. 콘텐츠의 중심)
                    4. 맥락 추가: 알게 된 배경 (팩트 나열이 아닌, 정보를 찾아보며 알게 된 전후 맥락)
                ...
                - 정보형 (에세이형 정보 템플릿 적용):
                    ...
                    4. 구조화된 정보: 최소한의 정리 (메모 느낌의 리스트 활용)
                    5. 적용 맥락: '누구에게 유용한가' 대신 '언제 떠오르는가'
                    6. 한계와 예외: 정보의 절대성 제거
                    7. 마무리 문장: 결론이 아닌 여운
                - 커머스형 (에세이형 커머스 템플릿 적용):
                    1. 첫 문장: 쓰게 된 이유 (후킹이 아닌 고백. "어느 날, 이게 필요하다는 생각이 들었습니다.")
                    2. 망설임: 바로 사지 않았다는 기록 (비슷한 것들 사이의 고민)
                    3. 선택의 이유: 기능을 경험으로 번역 (예: "밤에 켜도 집이 조용했습니다")
                    4. 사용 후 변화: 하루의 변화 (인생템 금지. "하루가 조금 덜 번거로워졌어요")
                ...
                STEP 4. 톤 앵커 고정: 1인칭 유지. 확신하는 어미("정답은 분명/결국 문제는") 제거. 과도한 감정어(분노/참담/충격) 금지. 
                STEP 5. 초안 작성: "저는 / 당시엔 / 나중에 보니" 등 과정과 흔들림을 기록하는 어조.
